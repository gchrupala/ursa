...
class Kernel:
    ...
    def __init__(self, label=label, leaf=leaf, children=children, ignore_terminals=False):
        self.label = label
        self.leaf = leaf
    ...
    def C(self, n1: TreeLike, n2: TreeLike, alpha: float=1.0) -> float:
        ...
        # both nodes are non-terminals and have same productions
            ...
            return alpha * product(1 + self.C(n1[i], n2[i], alpha=alpha) for i in range(len(self.children(n1))))
        else:
            return 0

    def __call__(self, t1: TreeLike, t2: TreeLike, alpha: float=1.0):
        """Returns the number of shared tree fragments between trees t1 and t2, discounted by alpha."""
        N = sum(self.C(n1, n2, alpha=alpha) for n1 in self.subtrees(t1) for n2 in self.subtrees(t2))
        return N
